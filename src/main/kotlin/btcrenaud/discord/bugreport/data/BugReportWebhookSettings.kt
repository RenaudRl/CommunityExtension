package btcrenaud.discord.bugreport.data
...
data class BugReportWebhookSettings(
    // The `enabled` flag is gone: an empty destination IS the disabled state. Two switches for
    // one effect always end up contradicting each other.
    @Help("Discord destination receiving the bug reports. Empty = no delivery.")
