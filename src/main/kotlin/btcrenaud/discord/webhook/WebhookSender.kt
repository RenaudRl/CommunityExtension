...
class WebhookSender(
    ...
    fun send(
        ...
        val payload = buildPayload(entry, content, embeds, threadName, appliedTagIds, pingRoleIds)
        ...
        httpClient.sendAsync(request, HttpResponse.BodyHandlers.ofString())
            .whenComplete { response, error ->
                when {
                    error != null ->
                        logger.warning("Failed to deliver webhook '${entry.name}': ${error.message}")
                    response.statusCode() !in 200..299 ->
                        logger.warning(
                            "Webhook '${entry.name}' rejected with HTTP ${response.statusCode()}: ${response.body().take(200)}"
                        )
                }
            }
    }
    ...
    private fun buildPayload(
        ...
        entry.username.takeIf { it.isNotBlank() }?.let { payload["username"] = it }
        entry.avatarUrl.takeIf { it.isNotBlank() }?.let { payload["avatar_url"] = it }
