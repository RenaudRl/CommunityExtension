package btcrenaud.community.bugreport.entries

import btcrenaud.community.CommunityExtension
import btcrenaud.community.bugreport.data.BugReportCategory
import btcrenaud.community.bugreport.data.BugReportMessages
import btcrenaud.community.bugreport.data.BugReportStatus
...
import btcrenaud.community.bugreport.data.defaultStatuses
...
class BugReportManifestEntry(
    ...
    override fun command(): DslCommand<CommandSourceStack> = command(commandName.ifBlank { "bugreport" }) {
        executePlayer { player ->
            CommunityExtension.bugReportSystem(id)?.openCategorySelection(player)
        }
        literal("mine") {
            executePlayer { player ->
                CommunityExtension.bugReportSystem(id)?.listOwnReports(player)
            }
        }
        literal("list") {
            withPermission(adminPermission)
            executes {
                CommunityExtension.bugReportSystem(id)?.listReports(sender, playerFilter = null)
            }
            word("player") { playerName ->
                executes {
                    CommunityExtension.bugReportSystem(id)?.listReports(sender, playerFilter = playerName())
                }
            }
        }
        literal("status") {
            ...
            word("reportId") { reportId ->
                word("statusId") { statusId ->
                    executes {
                        CommunityExtension.bugReportSystem(id)?.updateStatus(sender, reportId(), statusId())
                    }
                }
            }
        }
        literal("delete") {
            ...
            word("reportId") { reportId ->
                executes {
                    CommunityExtension.bugReportSystem(id)?.deleteReport(sender, reportId())
                }
            }
        }
    }
